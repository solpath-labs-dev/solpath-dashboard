...
**아임웹 위젯 · CORS** — 정적 프론트(`jsDelivr`)·iframe·`fetch`로 `…/exec` 를 부를 때의 정책·OPTIONS·토큰 주의는 전용 repo 문서 [solpath-dashboard-front: docs/IMWEB_CORS.md](https://github.com/eunsang9597/solpath-dashboard-front/blob/main/docs/IMWEB_CORS.md) (동일 본문이 `solpath-labs-dev` 에도 있음).

**메서드** — `GET` = OAuth·관리 콘솔 흐름(`Code.js` `doGet`). `POST` = 대시보드·동기 API(`HttpOpenSync.js` `doPost`).
...
*응답*은 `text/plain` `ok`가 아니라 **OAuth·토큰·리다이렉트** 위주(구버전 문서와 다름). 헬스는 Executions / 자체 `POST` `ping` 사용.

### 2.2 `POST` Open Sync (구현 — `HttpOpenSync.js`)

**URL** — Web App **배포** URL과 동일(예: `https://script.google.com/macros/s/…/exec`). **CORS** 응답 헤더 포함(브라우저 `fetch`).

**Property** — `SOLPATH_DASHBOARD_TOKEN` (빈면 모든 `action` 401). 프론트 `config.js` `DASHBOARD_SYNC_API_TOKEN`과 **동일**해야 함(정적·공개 배포는 토큰 노출 전제; 내부·제한 URL).

| `action` | 본문 `token` | 동작 | 성공 `data` |
|----------|--------------|------|-------------|
| `ping` | 필수(유효) | 연결·토큰 확인 | `{ name, version, actions: ['ping','syncOpenFull'] }` |
| `syncOpenFull` | 필수(유효) | `dbSyncOpenAll()` = members → products(1p) → orders | `{ members, products, orders }` (각 `dbSync*Open` 반환) |

**Request** — `Content-Type: application/x-www-form-urlencoded` (프리플라이트 최소화 권장).

```
action=syncOpenFull&token=<SOLPATH_DASHBOARD_TOKEN>
```
...
**OPTIONS** — `doOptions` (환경에 따라 프리플라이트; 미호출 시에도 `urlencoded` `POST`는 동작 가능).

**함수(코드) 매핑** — `action=syncOpenFull` → `dbSyncOpenAll()` → `dbSyncMembersOpen` → `dbSyncProductsOnePage` → `dbSyncOrdersOpen`.
...
| 2026-04-25 | **GAS** — `HttpOpenSync.js`: `POST` `action=ping|syncOpenFull` + `token` · CORS. `dbSyncOpenAll` 반환. Property `SOLPATH_DASHBOARD_TOKEN` |

이후부터는 **날짜당 한 줄** 위주로 쌓고, 상세는 [process.md](../process.md) 변경 이력에 남긴다.
