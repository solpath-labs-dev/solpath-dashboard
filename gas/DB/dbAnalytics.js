 ...
 */
function dbAnalyticsOrderLinesRawGet_(y, m) {
  var ssA;
  try {
    ssA = dbAnOpenOrThrow_();
  } catch (e) {
  ...
  if (!isFinite(m) || m < 1 || m > 12) {
    return { ok: false, error: { code: 'BAD_REQUEST', message: 'month는 1~12만 지원합니다.' } };
  }
  var sh = dbAnGetOrderLinesSheet_(ssA);
  var lr = sh.getLastRow();
  if (lr < 2) {
    return { ok: true, data: { rows: [], truncated: false } };
  }
  var w0 = DB_ANALYTICS_ORDER_LINE_HEADERS.length;
  var nR = Math.max(0, lr - 1);
  var vals = sh.getRange(2, 1, nR, w0).getValues();
  var out = [];
  var truncated = false;
  var j;
  for (j = 0; j < vals.length; j++) {
    var L = vals[j] || [];
    var ymd0 = dbAnAnyToSeoulYmd_(L[3]);
    if (!ymd0) {
      continue;
    }
    if (!dbAnYmdInScope_(ymd0, y, m)) {
      continue;
    }
    if (out.length >= DB_AN_ORDER_LINES_RAW_MAX) {
      truncated = true;
      break;
    }
    ...
    out.push({
      ...
      order_item_code: L[1] != null ? String(L[1]) : '',
      order_no: L[2] != null ? String(L[2]) : '',
      order_time: L[3],
      order_time_ymd: ymd0,
      prod_no: L[4] != null ? String(L[4]) : '',
      prod_name: L[5] != null ? String(L[5]) : '',
      line_net_amount: L[6],
