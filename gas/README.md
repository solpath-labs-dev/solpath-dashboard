...
- `Code.js` — Web App `doGet`: **선행** `?format=jsonp&callback=&action=` → `openSyncJsonpFromGet_` (`HttpOpenSync.js`); 그 외 `?code=` OAuth·리프레시·아임웹 UI.
- `HttpOpenSync.js` — Web App `doPost` / `doOptions`: 본문 `action=ping|syncOpenFull` → `dbSyncOpenAll()`. 브라우저·CORS·JSONP·시트 `getRange` 규칙 → [docs/GAS_WEBAPP_SHEETS.md](../docs/GAS_WEBAPP_SHEETS.md) · [docs/BACKEND_API.md](../docs/BACKEND_API.md) §2
